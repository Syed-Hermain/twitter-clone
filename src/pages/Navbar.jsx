...
const Navbar = ({ user }) => {
  ...
  return (
    <>
      ...
      {/* Mobile Sidebar */}
      <div className={`fixed inset-y-0 left-0 w-64 bg-blue-700 transform ${isOpen ? "translate-x-0" : "-translate-x-full"} transition duration-300 md:hidden z-50`}>
        <button className="absolute top-3 right-3 text-white" onClick={closeSidebar}>✖</button>
        <div className="p-4 flex flex-col space-y-4 text-white">
          <NavLink to="/" className="hover:text-gray-300" onClick={closeSidebar}>Home</NavLink>
          <NavLink to="/explore" className="hover:text-gray-300" onClick={closeSidebar}>Explore</NavLink>
          <NavLink to="/notifications" className="hover:text-gray-300" onClick={closeSidebar}>Notifications</NavLink>
          <hr className="border-gray-400 my-2" />
          <NavLink to="/profile" className="flex items-center space-x-2 hover:text-gray-300" onClick={closeSidebar}>
